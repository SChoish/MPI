...
Offline reinforcement learning must balance two goals: policy updates should stay near dataset-supported actions to keep value estimates reliable, yet meaningful gains often require moving beyond the behavior distribution. This repository implements **Multi-step Proximal Policy Improvement (MPI)**, a plug-in refinement mechanism that composes sequential **re-centered** proximal steps on a policy manifold. Each refinement is centered at the latest policy rather than the original data or behavior anchor, providing path-wise control of movement beyond dataset support.
...
The proximal step size $\tau$ is the implicit discretization step of this critic-induced policy flow, not only a regularization knob. Small $\tau$ produces conservative local refinements; larger $\tau$ permits farther movement but can increase discretization error and exposure to critic approximation error. MPI therefore studies a stability–improvement trade-off rather than guaranteeing monotonic improvement in true return under an imperfect offline critic.
