...
class GaussianMLP(nn.Module):
    ...
    def log_prob_actions(
        ...
        """Return [B] - log probability of actions
        ...
        Returns:
            [B] log probability of actions
        """
        mean, std = self.get_mean_std(params, states)
        
        # Normal distribution log_prob: -0.5 * ((x - mu) / sigma)^2 - log(sigma) - 0.5 * log(2*pi)
        # Sum over action dimension
        log_prob = -0.5 * jnp.sum(((actions - mean) / (std + 1e-8)) ** 2, axis=-1)
        log_prob = log_prob - jnp.sum(jnp.log(std + 1e-8), axis=-1)
        log_prob = log_prob - 0.5 * self.action_dim * jnp.log(2 * jnp.pi)
...
class TanhGaussianMLP(nn.Module):
    ...
    def log_prob_actions(
        ...
        """Return [B] - log probability of actions (with change of variables)
        
        Args:
            params: Actor parameters
            states: [B, state_dim]
            actions: [B, action_dim] (bounded in [-max_action, max_action])
        
        Returns:
            [B] log probability of actions
        """
        mean, log_std = self.apply(params, states)
        log_std = jnp.clip(log_std, self.log_std_min, self.log_std_max)
        std = jnp.exp(log_std)
        
        # actions: [-max_action, max_action] -> unbounded: atanh(actions/max_action)
        actions_scaled = actions / (self.max_action + 1e-8)
        actions_scaled = jnp.clip(actions_scaled, -0.9999, 0.9999)
        action_unbounded = 0.5 * (jnp.log1p(actions_scaled) - jnp.log1p(-actions_scaled))
        
        # Normal distribution log_prob in unbounded space
        log_p = -0.5 * jnp.sum(((action_unbounded - mean) / (std + 1e-8)) ** 2, axis=-1)
        log_p = log_p - jnp.sum(jnp.log(std + 1e-8), axis=-1)
        log_p = log_p - 0.5 * self.action_dim * jnp.log(2 * jnp.pi)
        
        # Change of variables: subtract log(1 - tanh^2)
        log_p = log_p - jnp.sum(jnp.log(1 - actions_scaled ** 2 + 1e-6), axis=-1)
...
class StochasticMLP(nn.Module):
    ...
    def log_prob_actions(
        ...
        Returns:
            [B] zeros (log_prob not available for StochasticMLP)
        """
        return jnp.zeros(actions.shape[0])
...
class DeterministicMLP(nn.Module):
    ...
    def log_prob_actions(
        ...
        Returns:
            [B] zeros (deterministic policy has no log_prob)
        """
        return jnp.zeros(actions.shape[0])


# ============================================================================
# FQL Flow Policy (Flow Matching 기반)
# ============================================================================
