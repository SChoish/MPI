# MPI (JAX implementation)
# JAX 기반 알고리즘에 MPI 구조 적용
...
def _parse_env_name(env_name: str) -> Tuple[str, str]:
    ...
    예: 'halfcheetah-medium-expert-v2' -> ('halfcheetah', 'medium_expert_v2')
    """
    parts = env_name.split("-")
    if len(parts) < 2:
        return (parts[0], "default") if parts else ("unknown", "default")
    env = parts[0]
    task = "_".join(parts[1:])
    return env, task
...
def _to_numpy_tree(pytree):
    """JAX/Flax pytree를 numpy 배열로 변환 (체크포인트 직렬화용)."""
    return jax.tree_util.tree_map(
        lambda x: np.asarray(x) if hasattr(x, "__array__") else x,
        pytree,
    )
...
class Config(BaseConfig):
    ...
    # epoch 기준 평가 주기 (없으면 eval_freq와 num_updates_on_epoch로부터 계산)
    eval_every: Optional[int] = None
    # True면 wandb 로깅 비활성화 (스크립트에서 --no_wandb 전달 시 사용)
    no_wandb: bool = False

    def __post_init__(self):
        super().__post_init__()
        if self.eval_every is None:
            self.eval_every = max(1, int(self.eval_freq // self.num_updates_on_epoch))
...
def update_multi_actor_gaussian(
    ...
    for i in range(num_actors):
        ...
        def actor_loss_fn(params: FrozenDict) -> Tuple[jax.Array, Metrics]:
            # Mixed actor types are allowed even in the Gaussian path.
            if actor_is_gaussian[i]:
                actions_i, std_i = actor_module_i.get_mean_std(params, batch["states"])
            else:
                actions_i = actor_module_i.apply(params, batch["states"])
                std_i = None

            bc_penalty = ((actions_i - batch["actions"]) ** 2).sum(-1)
            q_values = new_critic.apply_fn(new_critic.params, batch["states"], actions_i).min(0)
            lmbda = 1.0
            if normalize_q:
            ...
            if i == 0:
                ...
                if actor_is_gaussian[i] and actor_is_gaussian[i - 1]:
                    # Closed form W2 for Gaussian-Gaussian pairs
                    mean_ref, std_ref = ref_actor_module.get_mean_std(ref_actor.params, batch["states"])
                    mean_ref = jax.lax.stop_gradient(mean_ref)
                    std_ref = jax.lax.stop_gradient(std_ref)
                    w2_dist = closed_form_w2_gaussian(actions_i, std_i, mean_ref, std_ref).mean()
                else:
                    # Fallback for mixed pairs: L2 distance between deterministic actions
                    if actor_is_gaussian[i - 1]:
                        ref_actions, _ = ref_actor_module.get_mean_std(ref_actor.params, batch["states"])
                    else:
                        ref_actions = ref_actor_module.apply(ref_actor.params, batch["states"])
                    ref_actions = jax.lax.stop_gradient(ref_actions)
                    w2_dist = jnp.sum((actions_i - ref_actions) ** 2, axis=-1).mean()
...
def main(config: Config):
    ...
    wandb.init(
        ...
        name=config.name,
        id=str(uuid.uuid4()),
        mode="disabled" if getattr(config, "no_wandb", False) else "online",
    )
    try:
    ...
    split_keys = jax.random.split(key, config.num_actors + 2)
    key = split_keys[0]
    critic_key = split_keys[1]
    actor_keys = split_keys[2:]
    ...
    def make_actor_action_fn(actor_idx: int):
        ...
        def _action_fn(params: FrozenDict, obs: jax.Array) -> jax.Array:
            if is_gaussian:
                ...
                # StochasticMLP: use z=0 for deterministic. obs 1D면 z (action_dim,), 2D면 z (batch, action_dim)
                if obs.ndim == 1:
                    z_zero = jnp.zeros((actor_module.action_dim,), dtype=obs.dtype)
                else:
    ...
    print("---------------------------------------", flush=True)
    print(f"Training MPI (JAX), Env: {config.dataset_name}, Seed: {config.train_seed}", flush=True)
    print(f"  Actors: {config.num_actors}, W2 weights (Actor1+): {config.w2_weights}", flush=True)
    print("  Actor0: 원래 알고리즘 loss만 사용", flush=True)
    print("  Actor1+: 알고리즘 loss + W2 distance", flush=True)
    print("    - Gaussian actors: Closed form W2 (||μ1-μ2||² + ||σ1-σ2||²)", flush=True)
    print("    - TanhGaussian/Stochastic actors: Sinkhorn distance", flush=True)
    print("    - Stochastic (non-Gaussian): Sinkhorn distance", flush=True)
    print("    - Deterministic: L2 distance", flush=True)
    print(f"  Actor types: {[config.actor_configs[i].get('type', 'deterministic') for i in range(config.num_actors)]}", flush=True)
    print("---------------------------------------", flush=True)

    # Checkpoint 저장 디렉토리: results/{algorithm}/{env}/{task}/seed_{seed}/w{W}/checkpoints (weight별 분리)
    env_name, task_name = _parse_env_name(config.dataset_name)
    algorithm_name = "rebrac"
    w2_val = int(config.w2_weights[0]) if config.w2_weights else 10
    checkpoint_dir = os.path.join(
        "results", algorithm_name, env_name, task_name,
        f"seed_{config.train_seed}", f"w{w2_val}", "checkpoints",
    )
    os.makedirs(checkpoint_dir, exist_ok=True)

    def save_checkpoint(carry: Dict, global_step: int, suffix: str = ""):
        """체크포인트 저장 (critic + 모든 actor params/target_params)."""
        ckpt = {
            "global_step": global_step,
            "critic_params": _to_numpy_tree(carry["critic"].params),
            "critic_target_params": _to_numpy_tree(carry["critic"].target_params),
        }
        for i in range(config.num_actors):
            ckpt[f"actor_{i}_params"] = _to_numpy_tree(carry["actors"][i].params)
            ckpt[f"actor_{i}_target_params"] = _to_numpy_tree(carry["actors"][i].target_params)
        path = os.path.join(checkpoint_dir, f"model_step{global_step}{suffix}.ckpt")
        with open(path, "wb") as f:
            pickle.dump(ckpt, f, protocol=pickle.HIGHEST_PROTOCOL)
        print(f"Checkpoint 저장 완료: {path}", flush=True)
    ...
    for epoch in range(config.num_epochs):
        ...
        wandb.log(
            {"epoch": epoch, **{f"MPI_JAX/{k}": v for k, v in mean_metrics.items()}}
        )

        global_update_step = (epoch + 1) * config.num_updates_on_epoch
        # 500k timestep마다 체크포인트 저장 (main과 동일)
        if global_update_step % 500000 == 0:
            save_checkpoint(update_carry, global_update_step, f"_step{global_update_step}")
        if epoch == 0 or (global_update_step % config.eval_freq == 0) or epoch == config.num_epochs - 1:
            ...
            for actor_idx in range(config.num_actors):
                ...
                norm_avg = float(np.mean(normalized_score))
                actor_results.append({"raw_avg": raw_avg, "norm_score": norm_avg})
                wandb.log(
                    {
                        "epoch": epoch,
            ...
            for actor_idx in range(config.num_actors):
                result = actor_results[actor_idx]
                print(
                    f"  Actor {actor_idx} - Raw: {result['raw_avg']:.3f}, D4RL score: {result['norm_score']:.1f}",
                    flush=True,
                )
            print("---------------------------------------", flush=True)

    total_steps = config.num_epochs * config.num_updates_on_epoch
    print("\nTraining completed! Saving final checkpoint before evaluation...", flush=True)
    print("=" * 60, flush=True)
    save_checkpoint(update_carry, total_steps, "_final")

    final_eval_runs = getattr(config, "final_eval_runs", 5)
    final_eval_episodes = getattr(config, "final_eval_episodes", 100)
    print("\n" + "=" * 60, flush=True)
    print("======== Final Evaluation (all actors) ========", flush=True)
    print("=" * 60, flush=True)
    for actor_idx in range(config.num_actors):
        print(f"\n======== Final Evaluation: Actor {actor_idx} ========", flush=True)
        det_scores = []
        stoch_scores = []
        for run_idx in range(final_eval_runs):
            action_fn = make_actor_action_fn(actor_idx)
            det_returns = evaluate(
                eval_env,
                update_carry["actors"][actor_idx].params,
                action_fn,
                final_eval_episodes,
                seed=config.eval_seed + actor_idx * 1000 + run_idx * 100,
            )
            det_norm = eval_env.get_normalized_score(det_returns) * 100.0
            det_scores.append(float(np.mean(det_norm)))
            # JAX 평가 경로는 deterministic action_fn 기준으로 고정
            stoch_scores.append(float(np.mean(det_norm)))

        det_scores = np.asarray(det_scores, dtype=np.float32)
        stoch_scores = np.asarray(stoch_scores, dtype=np.float32)
        print(
            f"[FINAL] Deterministic: mean={det_scores.mean():.3f}, std={det_scores.std():.3f} "
            f"over {final_eval_runs}x{final_eval_episodes}",
            flush=True,
        )
        print(
            f"[FINAL] Stochastic:   mean={stoch_scores.mean():.3f}, std={stoch_scores.std():.3f} "
            f"over {final_eval_runs}x{final_eval_episodes}",
            flush=True,
        )
