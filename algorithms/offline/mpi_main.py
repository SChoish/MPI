...
class ActorCreationConfig:
    ...
    mpi_default_type: str = "stochastic"
    mpi_default_tanh_mean: bool = True
...
def _build_actor0_config(actor0: nn.Module, is_stochastic: bool, is_gaussian: bool) -> Actor0Config:
    ...
    mpi_type, kwargs = _infer_actor0_type(actor0)
    return Actor0Config(
        type=mpi_type,
        kwargs=kwargs,
...
def _infer_actor0_type(actor0: nn.Module) -> Tuple[str, dict]:
    """Actor0로부터 MPI 타입 및 kwargs 추론. type 미지정 시 Actor1+ 초기화용."""
...
def _create_actors(
    ...
    """config로 actor 생성: actor0=create_actor0(), actor1+=MPI policies.
    ...
    # actor1+: MPI policies
    num_mpi = num_actors - 1
    if num_mpi <= 0:
        return actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config

    default_type = actor_creation.mpi_default_type
    default_tanh_mean = actor_creation.mpi_default_tanh_mean
    actor0_ref = actors[0]
    ...
    if cfg_1plus is None or len(cfg_1plus) == 0:
        cfg_1plus = [{"type": fallback_type, "_from_stored": True, **fallback_kw} for _ in range(num_mpi)]
    else:
...
def _get_config_path(config: TrainConfig) -> str:
    """config 파일 경로 생성: configs/offline/mpi/{algorithm}/{env}/{task}/seed_{seed}/config.yaml"""
    env, task = _parse_env_name(config.env)
    config_dir = Path("configs/offline/mpi") / config.algorithm / env / task / f"seed_{config.seed}"
...
def _create_config_file(config: TrainConfig) -> str:
    ...
    # 알고리즘별 주석 템플릿
    algorithm_comments = {
        "iql": "# MPI with IQL algorithm\n# IQL의 구조(V, Q, Actor)를 그대로 사용하되, Actor만 multi-actor",
        "td3_bc": "# MPI with TD3_BC algorithm",
        "cql": "# MPI with CQL algorithm",
        "awac": "# MPI with AWAC algorithm",
        "sac_n": "# MPI with SAC-N algorithm",
        "edac": "# MPI with EDAC algorithm",
    }
    
    comment = algorithm_comments.get(config.algorithm, f"# MPI with {config.algorithm.upper()} algorithm")
...
# mpi_main 또는 각 알고리즘 파일에서 import
...
def train(config: TrainConfig):
    ...
    print("=" * 60, flush=True)
    print("MPI Training Started", flush=True)
    ...
    if config.algorithm == "iql":
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            ...
            ActorCreationConfig(create_actor0=_iql_actor0, mpi_default_type="gaussian", mpi_default_tanh_mean=True),
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            state_dim, action_dim, max_action, config.num_actors,
            config.actor_configs,
            ActorCreationConfig(create_actor0=_td3_actor0, mpi_default_type="deterministic"),
            config.device, config.actor_lr,
        )
        base_actor0 = actors[0]
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            state_dim, action_dim, max_action, config.num_actors,
            config.actor_configs,
            ActorCreationConfig(create_actor0=_cql_actor0, mpi_default_type="tanh_gaussian"),
            config.device, config.actor_lr,
        )
        base_actor0 = actors[0]
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            state_dim, action_dim, max_action, config.num_actors,
            config.actor_configs,
            ActorCreationConfig(create_actor0=_awac_actor0, mpi_default_type="gaussian", mpi_default_tanh_mean=False),
            config.device, config.actor_lr,
        )
        base_actor0 = actors[0]
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            state_dim, action_dim, max_action, config.num_actors,
            config.actor_configs,
            ActorCreationConfig(create_actor0=_sacn_actor0, mpi_default_type="tanh_gaussian"),
            config.device, config.actor_lr,
        )
        base_actor0 = actors[0]
        ...
        actors, actor_targets, actor_optimizers, actor_is_stochastic, actor_is_gaussian, stored_config = _create_actors(
            state_dim, action_dim, max_action, config.num_actors,
            config.actor_configs,
            ActorCreationConfig(create_actor0=_edac_actor0, mpi_default_type="tanh_gaussian"),
            config.device, config.actor_lr,
        )
        base_actor0 = actors[0]
    ...
    print("---------------------------------------", flush=True)
    print(f"Training MPI ({config.algorithm.upper()}), Env: {config.env}, Seed: {config.seed}", flush=True)
    ...
    checkpoint_dir = os.path.join(
        "results", config.algorithm, env_name, task_name,
        f"seed_{config.seed}", f"w{w2_val}", "checkpoints",
    )
    ...
    # 학습 완료 후 최종 평가 (MPI 형식: runs x episodes)
    ...
    # 파일명: {환경}_{알고리즘}_{actor_type}_{actor_num}_{w2weight}_{seed}.json
    import json
    ...
    actor_type = "gaussian"
    if config.actor_configs and len(config.actor_configs) > 0:
        actor_type = config.actor_configs[0].get("type", "gaussian")
    w2weight = int(config.w2_weights[0]) if config.w2_weights else 10
    env_underscore = config.env.replace("-", "_")
    log_file = os.path.join(log_dir, f"{env_underscore}_{config.algorithm}_{actor_type}_{config.num_actors}_{w2weight}_{config.seed}.json")
