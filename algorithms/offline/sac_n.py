...
def Actor(state_dim: int, action_dim: int, hidden_dim: int, max_action: float = 1.0):
    """SAC-N policy: networks.TanhGaussianMLP (separate_mu_logstd, sac_edac_init)"""
...
class SACN:
    ...
    def _alpha_loss(self, state: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            action, action_log_prob = get_action(self.actor, state, deterministic=False, need_log_prob=True)
            if action_log_prob is None:
                action_log_prob = torch.zeros(action.size(0), device=action.device)
    ...
    def compute_energy_function(
        ...
        q_value_min = self.critic(state, pi_i).min(0).values
        return (self.alpha * log_pi_i - q_value_min).mean()
    ...
    def _critic_loss(
        ...
        with torch.no_grad():
            next_action, next_action_log_prob = get_action(
                self.actor, next_state, deterministic=False, need_log_prob=True
            )
            if next_action_log_prob is None:
                next_action_log_prob = torch.zeros(next_action.size(0), device=next_action.device)
