...
from algorithms.networks import DeterministicMLP as Actor
...
class TD3_BC:
    def __init__(
        ...
        alpha: float = 2.5,
        device: str = "cpu",
        actor_target: Optional[nn.Module] = None,
    ):
        self.actor = actor
        self.actor_target = actor_target if actor_target is not None else copy.deepcopy(actor)
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        if self.total_it % self.policy_freq == 0:
            # Compute actor loss
            pi, _ = get_action(self.actor, state, deterministic=True)
